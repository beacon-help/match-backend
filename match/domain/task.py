...
class Task:
    ...
    def create_task(cls, owner: User, title: str, description: str) -> "Task":
        return cls(id=None, status=TaskStatus.OPEN, owner=owner, helper=None, title=title, description=description)
    ...
    # def close(self, owner: User) -> None:
    #     self._validate_owner(owner)
    #     if self.status not in (TaskStatus.OPEN, TaskStatus.PENDING, TaskStatus.APPROVED):
    #         raise Exception(f"Cannot close task in {self.status} state.")
